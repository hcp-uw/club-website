...
describe("Testing updateProject", () => {
  ...
  it("should update an existing project's 'Completed' field and return true", async () => {
    ...
    const result = await updateProject(projectName, key, value, "Test/Projects");
    assert.isTrue(result, "Expected updateProject to return true");


    const updatedProject = (await getProjectByName(projectName, "Test/Projects"))[0];

    assert.strictEqual(updatedProject.Completed, value, "Expected 'Completed' field to be updated");
  });
  ...
  it("should update an existing project's 'Description' field and return true", async () => {
    ...
    const updatedProject = (await getProjectByName(projectName, "Test/Projects"))[0];
    assert.strictEqual(updatedProject.Description, value, "Expected 'Description' field to be updated");
  });
  ...
  it("should return false when required parameters are missing", async () => {
    ...
    let result = await updateProject(undefined, key, value, "Test/Projects");
    assert.isFalse(result, "Expected updateProject to return false");


    const projectName = "Test Project";
    result = await updateProject(projectName, undefined, value, "Test/Projects");
    assert.isFalse(result, "Expected updateProject to return false");


    result = await updateProject(projectName, key, undefined, "Test/Projects");
    assert.isFalse(result, "Expected updateProject to return false");
  });
