...
const getInput = (
    ...
    if (key.indexOf("Date") !== -1 && typeof val === "number") {
        ...
        const adjDate =
            date !== undefined ? date.toISOString().slice(0, 16) : "";
...
const parseValue = (key: string, value: Value) => {
    if (key.indexOf("Date") !== -1) {
        ...
        else if (typeof value === "number" && isNaN(value)) convertedValue = -1;
...
const Item: React.FC<ItemProps> = (props: ItemProps) => {
    const { item, index, handleInputChange, handleSave, handleDelete, T } =
        props;
...
const Form: React.FC<FormProps<_Person | _Project | _Event>> = ({
    data,
    handleSave,
    handleDelete,
    T,
    ...
    return (
        <div>
            {[...Array(Math.ceil(currentData.length / pageSize))].map(
                (_, i) => (
                    <Button
    ...
    return show ? (
        <Box key={index} mb={4}>
            ...
            <Button
                ...
                onClick={() => {
                    handleSave(transformData(currentData)).then(
                        (success: boolean) => {
                            if (success === false) alert("Error saving item");
...
const AdminForm: React.FC<AdminFormProps> = ({
    ...
    const getCreateFunc = (T: "Event" | "Person" | "Project") => {
        switch (T) {
            case "Event":
                return (item: _Item) => EventService.createNewEvent(item);
            case "Person":
                return (item: _Item) => LeadService.createNewLead(item);
            case "Project":
                return (item: _Item) => ProjectService.createNewProject(item);
        }
    };
    ...
    const boxStyle = {
        /* border: '1px solid white', borderRadius: '20px',*/ padding: "15px",
        margin: "10px",
    };
